...
```{r setup-basque, warning=FALSE, message=FALSE, echo=TRUE, results='hide'}
library(Synth)
...
```{r basque-plots, fig.height=4, fig.width=6}
...
```{r basque-plotscl, fig.height=4, fig.width=6}
...
```{r setup-gscm, warning=FALSE, message=FALSE, echo=TRUE, results='hide'}
...
```{r plot-gscm, fig.height=4, fig.width=6}
...
```{r fig_syn, fig.width=8, fig.height=4.5, echo=TRUE, fig.align="center"}
...
```{r synthdid-setup, echo=TRUE, message=FALSE}
...
```{r synd2, fig.cap='Simulated Sharp RDD with Scholarship Cutoff', fig.align='center', echo=FALSE, warning=FALSE, message=FALSE}
library(tidyverse)
set.seed(42)
...
dat <- dat %>%
  mutate(
    treated = if_else(score > cutoff, 1, 0),
    potential_outcome = 20 + 30 * treated + 0.8 * score + rnorm(n(), 0, 15)
  )

# Fit separate linear models on each side of the cutoff
left_lm <- lm(potential_outcome ~ score, data = filter(dat, score <= cutoff))
right_lm <- lm(potential_outcome ~ score, data = filter(dat, score > cutoff))

# Predict outcomes at the cutoff
y_left <- predict(left_lm, newdata = tibble(score = cutoff))
y_right <- predict(right_lm, newdata = tibble(score = cutoff))

#Plot
ggplot(dat, aes(x = score, y = potential_outcome)) +
  geom_point(alpha = 0.5, color = "steelblue") +  # Data points

  # Regression lines left and right of cutoff
  stat_smooth(data = filter(dat, score <= cutoff),
              method = "lm", se = FALSE, color = "darkorange") +
  stat_smooth(data = filter(dat, score > cutoff),
              method = "lm", se = FALSE, color = "darkorange") +

  # Bandwidth region (±h)
  annotate("rect", xmin = cutoff - bandwidth, xmax = cutoff + bandwidth,
           ymin = -Inf, ymax = Inf, alpha = 0.1, fill = "lightblue") +

  # Vertical cutoff line
  geom_vline(xintercept = cutoff, color = "firebrick", linetype = "dashed", linewidth = 1) +

  # Label for cutoff
  annotate("text", x = cutoff, y = min(dat$potential_outcome) + 5,
           label = "Cutoff = 75", vjust = 0, hjust = 0.5, size = 4, color = "firebrick") +

  # Treatment effect segment at cutoff
  geom_segment(
    aes(x = cutoff, xend = cutoff, y = y_left, yend = y_right),
    color = "yellow", size = 1.2
  ) +

  # h labels
  annotate("text", x = cutoff - bandwidth / 2, y = quantile(dat$potential_outcome, 0.05),
           label = "h", vjust = -0.5, size = 3.5, color = "black") +
  annotate("text", x = cutoff + bandwidth / 2, y = quantile(dat$potential_outcome, 0.05),
           label = "h", vjust = -0.5, size = 3.5, color = "black") +

  # Arrows for h
  annotate("segment", x = cutoff - bandwidth, xend = cutoff,
           y = quantile(dat$potential_outcome, 0.05),
           yend = quantile(dat$potential_outcome, 0.05),
           arrow = arrow(ends = "both", length = unit(0.07, "inches")), color = "black") +
  annotate("segment", x = cutoff, xend = cutoff + bandwidth,
           y = quantile(dat$potential_outcome, 0.05),
           yend = quantile(dat$potential_outcome, 0.05),
           arrow = arrow(ends = "both", length = unit(0.07, "inches")), color = "black") +
  ...
  labs(
    x = "Test Score",
    y = "Outcome (e.g., Expected Earnings)"
  ) +
  theme_minimal()
...
```{r synd3, fig.cap='RD Plot with Triangular Kernel and CCT Bandwidth (Local Linear Fit)', warning=FALSE, message=FALSE, cache=TRUE}
library(rdrobust)
...
```{r synd4, fig.cap='Estimated Density of the Running Variable around the Cutoff (McCrary Test)', echo=FALSE, warning=FALSE, message=FALSE}
library(rddensity)
density_test <- rddensity(X = x, c = 0, fitselect="restricted", vce="plugin")
#summary(density_test)
invisible(rdplotdensity(density_test, X = x))
```
