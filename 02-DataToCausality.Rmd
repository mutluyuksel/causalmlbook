...
calculate_stats <- function(x, y) {
  c(mean_x = formatC(mean(x), format = "f", digits = 3), 
    ...
    covariance = formatC(cov(x, y), format = "f", digits = 3),
...
```{r anscombe-plot, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Anscombe's Quartet: Four Datasets with Identical Statistics"}
...
cols <- c("darkorange", "dodgerblue", "forestgreen", "purple")

# Group I
plot(anscombe$x1, anscombe$y1, main = "Group I", xlab = "X1", ylab = "Y1",
     pch = 16, col = cols[1], xlim = xlim, ylim = ylim)
abline(lm(y1 ~ x1, data = anscombe), col = cols[1], lty = 1, lwd = 2)

# Group II
plot(anscombe$x2, anscombe$y2, main = "Group II", xlab = "X2", ylab = "Y2",
     pch = 16, col = cols[2], xlim = xlim, ylim = ylim)
abline(lm(y2 ~ x2, data = anscombe), col = cols[2], lty = 1, lwd = 2)

# Group III
plot(anscombe$x3, anscombe$y3, main = "Group III", xlab = "X3", ylab = "Y3",
     pch = 16, col = cols[3], xlim = xlim, ylim = ylim)
abline(lm(y3 ~ x3, data = anscombe), col = cols[3], lty = 1, lwd = 2)

# Group IV
plot(anscombe$x4, anscombe$y4, main = "Group IV", xlab = "X4", ylab = "Y4",
     pch = 16, col = cols[4], xlim = xlim, ylim = ylim)
abline(lm(y4 ~ x4, data = anscombe), col = cols[4], lty = 1, lwd = 2)
```
...
```{r nonlinear-plot, warning=FALSE, message=FALSE, fig.cap="Non-linear Relationship Between x and y"}
