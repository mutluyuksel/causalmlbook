...
```{r para1, echo=TRUE, warning=FALSE}
...
```{r para2, echo=TRUE, warning=FALSE}
...
```{r para3, warning=FALSE, message=FALSE, echo=FALSE, fig.cap="S-shaped curve of the logistic function"}
set.seed(1) 
n <- 500 
x = rnorm(n, 0, 2) 
sigma <- 1 / (1 + exp(-x)) 
plot(sigma ~ x, col = "green", cex.axis = 0.7) 
```
...
```{r para4, warning=FALSE, message=FALSE, fig.cap="Log-odds transformation of predicted probabilities"} 
p_x <- sigma 
Logit <- log(p_x / (1 - p_x)) 
...
```{r para5, warning=FALSE, message=FALSE}
...
```{r para6, warning=FALSE, message=FALSE}
lpm <- lm(y ~ x, data = data)
summary(lpm)
```

```{r para7, warning=FALSE, message=FALSE, fig.cap="True vs. estimated probabilities from LPM"}
...
```{r para7bw, warning=FALSE, message=FALSE, , eval=FALSE, fig.cap="True vs. estimated probabilities from LPM"}
#Plot the fitted probabilities against the true values
...
```{r para8, warning=FALSE, message=FALSE}
logis <- glm(y ~ x, data = data, family = binomial)
summary(logis)
```

```{r para9, warning=FALSE, message=FALSE, fig.cap="True vs. estimated probabilities from logistic regression"}
...
```{r para9bw, warning=FALSE, message=FALSE, eval=FALSE, fig.cap="True vs. estimated probabilities from logistic regression"}
#Plot the fitted probabilities for logistic regression
