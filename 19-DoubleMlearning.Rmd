...
```{r dml1, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r dml2, warning=FALSE, message=FALSE, cache=TRUE}
# Load required libraries
library(glmnet)      # For Lasso regression
library(MASS)        # For generating multivariate normal data
...
```{r setup-extra, include=FALSE, cache=TRUE}
# Load required libraries
library(glmnet)      # For Lasso regression
library(MASS)        # For generating multivariate normal data
library(caret)       # For cross-validation
...
```{r dml3, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r dml4, include=FALSE}
# THIS IS FOR US; TO COMPARE IN THE FUTURE 3A and 3B
# Load required libraries
library(glmnet)      # For Lasso regression
library(MASS)        # For generating multivariate normal data
library(caret)       # For cross-validation
...
# === Step 3A: Compute Plug-in Lasso Penalty ===
c <- 1.1  # Assumed scaling constant
gamma <- 0.1  # Assumed probability threshold
sigma_hat <- sd(Y)  # Estimated noise standard deviation from Y

# Compute the plug-in penalty
lambda_plugin <- c * sigma_hat * sqrt(N) * qnorm(1 - gamma / (2 * p))

# Display the plug-in lambda value
cat("Plug-in Lasso penalty (lambda):", lambda_plugin, "\n")

# === Step 3B: Compute a Fixed `lambda.min` Using Cross-Validation (Baseline) ===
cv_lasso_y <- cv.glmnet(X, Y, family = "gaussian", alpha = 1, nfolds = 10)  # Lasso for Y
lambda_fixed_y <- cv_lasso_y$lambda.min  # Store optimal lambda for outcome model

cv_lasso_d <- cv.glmnet(X, D, family = "binomial", alpha = 1, nfolds = 10)  # Lasso for D
lambda_fixed_d <- cv_lasso_d$lambda.min  # Store optimal lambda for propensity model

# === Step 4: Cross-Fitting with Plug-in Lambda for Y, D ===
Y_resid <- rep(NA, N)
D_resid <- rep(NA, N)
...
for (k in 1:K) {
    ...
    # ---- Fit Lasso for Outcome Model (Y ~ X) using Plug-in Lambda ----
    lasso_y <- glmnet(X_train, Y_train, family = "gaussian", alpha = 1, lambda = lambda_plugin)
    Y_hat_val <- predict(lasso_y, X_val)

    # ---- Fit Lasso for Treatment Model (D ~ X) using Plug-in Lambda ----
    lasso_d <- glmnet(X_train, D_train, family = "binomial", alpha = 1, lambda = lambda_plugin)
    D_hat_val <- predict(lasso_d, X_val, type = "response")
...
```{r dml6, warning=FALSE, message=FALSE, cache=TRUE}
# Load required libraries
library(MASS)       # For generating multivariate normal data
library(caret)      # For cross-validation
library(estimatr)   # For HC3 robust SEs (lm_robust)
...
```{r dml7, warning=FALSE, message=FALSE, cache=TRUE}
# Load required libraries
library(MASS)       # For generating multivariate normal data
library(caret)      # For cross-validation
library(estimatr)   # For HC3 robust SEs (lm_robust)
...
```{r dml8, warning=FALSE, message=FALSE, cache=TRUE}
