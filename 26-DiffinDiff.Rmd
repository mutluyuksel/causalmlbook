...
df <- data.frame(
  Group = c("Treated ($D=1$)", "Untreated ($D=0$)"),
  `Pre-Treatment ($T=0$)` = c("$E[Y_i(0) \\mid D=1]$", "$E[Y_i(0) \\mid D=0]$"),
  `Post-Treatment ($T=1$)` = c("$E[Y_i(1) \\mid D=1]$", "$E[Y_i(1) \\mid D=0]$")
)

colnames(df) <- c(
  "<span style='font-weight:normal;'>Group</span>",
  "<span style='font-weight:normal;'>Pre-Treatment ($T=0$)</span>",
  "<span style='font-weight:normal;'>Post-Treatment ($T=1$)</span>"
)

kable(df, escape = FALSE, caption = "Group Mean Outcomes by Treatment and Time")
```
...
df <- data.frame(
  Group = c("Treated (<span style='font-size:small;'>$D=1$</span>)", "Untreated (<span style='font-size:small;'>$D=0$</span>)", "Difference (<span style='font-size:small;'>T - U</span>)"),
  Pre = c("$\\alpha + \\beta_1$", "$\\alpha$", "$\\beta_1$"),
  Post = c("$\\alpha + \\beta_1 + \\beta_2 + \\delta$", "$\\alpha + \\beta_2$", "$\\beta_1 + \\delta$"),
  Diff = c("$\\beta_2 + \\delta$", "$\\beta_2$", "**DiD = $\\delta$**")
)

colnames(df) <- c(
  "<span style='font-weight:normal;'> </span>",
  "<span style='font-weight:normal;'>Pre-Treatment<br>($T{=}0$)</span>",
  "<span style='font-weight:normal;'>Post-Treatment<br>($T{=}1$)</span>",
  "<span style='font-weight:normal;'>Difference<br>(Post - Pre)</span>"
)

kable(df, escape = FALSE, caption = "Expected Outcomes from DiD Regression Model")
```
...
df <- data.frame(
  Group = rep(c("Treated_Pre", "Treated_Post", "Comparison",
                "Counterfactual_Pre", "Counterfactual_Post"), each = 2),
  Time = c(1, 1.5, 1.5, 2, 1, 2, 1, 1.5, 1.5, 2), # 1=Pre, 1.5=Treatment, 2=Post
  Outcome = c(26, 28, 28, 32, 21, 25, 26, 28, 28, 30) 
)

# Define mathematical notation as labels (for post-treatment differences)
notation_labels <- c(
  "alpha + beta[1]", "",  # Treated Pre 
  "", "alpha + beta[1] + beta[2] + delta",  # Treated Post 
  "alpha", "alpha + beta[2]",  # Comparison Pre (empty)
  "", "",  # Comparison Post (label at final point)
  "", "alpha + beta[1] + beta[2]"  # Counterfactual Post 
)


# Define colors
colors <- c("Treated_Pre" = "blue", "Treated_Post" = "blue", 
            "Comparison" = "red", "Counterfactual_Pre" = "blue",
            "Counterfactual_Post" = "green")

# Plot
ggplot(df, aes(x = Time, y = Outcome, group = Group, color = Group)) +
  ylim(20, 35) +
  geom_vline(xintercept = 1.5, linetype ="solid", color ="black", size=0.5) + 
  geom_line(data = subset(df, Group %in% c("Treated_Pre", 
                                          "Counterfactual_Pre")), size = 1) +  
  geom_line(data = subset(df, Group == "Treated_Post"), size = 1) +  
  geom_line(data = subset(df, Group == "Counterfactual_Post"), 
            size = 1, linetype = "dashed") +  
  geom_line(data = subset(df, Group == "Comparison"), size = 1) + 
  geom_point(size = 2, aes(fill = Group), shape = 21, color = "black") +
  geom_text(aes(label = ifelse(Time == 1.5 & Outcome == 28, "", Outcome)), 
            vjust = 1.5, size = 3, color = "black") +
  geom_text(aes(label = notation_labels), vjust = -0.5, size=5, parse=TRUE) + 
  scale_x_continuous(limits = c(0.8, 2.2), breaks = c(1, 1.5, 2), 
          labels = c("Pre-intervention", "Treatment", "Post-intervention")) + 
...
```{r DinD1, warning=FALSE, message=FALSE, cache=TRUE}
