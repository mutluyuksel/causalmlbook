...
```{r nn2bw, warning=FALSE, message=FALSE,eval=FALSE}
ols <- lm(y ~ x + I(x^2) + I(x^3))
plot(x, y, main="Polynomial: M = 3",  col= "grey")
...
```{r nn3, warning=FALSE, message=FALSE}
# Parts
...
```{r nn3bw, eval=FALSE, warning=FALSE, message=FALSE}
# Parts
first <- ols$coefficients[2]*x
second <- ols$coefficients[3]*x^2
third <- ols$coefficients[4]*x^3
...
```{r nnskip1, warning=FALSE, message=FALSE}
a = c(1.5, 9, 3)
b = c(-20,-14,-8)
beta = c(15, 25,-40)
int = 3
...
```{r nn4-fixed, warning=FALSE, message=FALSE, cache=TRUE}
library(neuralnet)
...
```{r nn6bw, warning=FALSE, message=FALSE,eval=FALSE}
X <- cbind(1, x)
...
```{r nn8, echo=FALSE, out.width = '65%', out.height='65%'}
...
```{r nn7a, cache=TRUE,  eval=FALSE, include=FALSE}
n <- 200
set.seed(1)
x <- sort(runif(n))
Y <- sin(12*(x + 0.2))/(x + 0.2) + rnorm(n)/2
...
while (converged == F) {
  ...
  a1_new<-a1-((0.005/n)*sum((Y-yhat)*(-beta1*exp(-z1)/((1+exp(-z1))^2))))
  a2_new<-a2-((0.005/n)*sum((Y-yhat)*(-beta2*exp(-z2)/((1+exp(-z2))^2))))
  a3_new<-a3-((0.005/n)*sum((Y-yhat)*(-beta3*exp(-z3)/((1+exp(-z3))^2))))
  b1_new<-b1-((0.005/n)*sum((Y-yhat)*(-beta1*x*exp(-z1)/((1+exp(-z1))^2))))
  b2_new<-b2-((0.005/n)*sum((Y-yhat)*(-beta2*x*exp(-z2)/((1+exp(-z2))^2))))
  b3_new<-b3-((0.005/n)*sum((Y-yhat)*(-beta3*x*exp(-z3)/((1+exp(-z3))^2))))
...
```{r nn7a-changed, cache=TRUE, echo=TRUE, eval=FALSE, include=FALSE}
n <- 200
set.seed(1)
x <- sort(runif(n))
Y <- sin(12*(x + 0.2))/(x + 0.2) + rnorm(n)/2
...
max_iter <- 50000      # iteration cap

# initial predictions and MSE
z1  <- a1 + b1*x;      sig1 <- 1/(1 + exp(-z1))
z2  <- a2 + b2*x;      sig2 <- 1/(1 + exp(-z2))
z3  <- a3 + b3*x;      sig3 <- 1/(1 + exp(-z3))
yhat <- alpha + beta1*sig1 + beta2*sig2 + beta3*sig3
prev_MSE <- mean((Y - yhat)^2)
...
for (iter in seq_len(max_iter)) {
  ...
  # gradients
  err  <- Y - yhat
  grad_alpha <- -2 * mean(err)
  grad_beta1 <- -2 * mean(err * sig1)
  grad_beta2 <- -2 * mean(err * sig2)
  grad_beta3 <- -2 * mean(err * sig3)

  ds1 <- sig1 * (1 - sig1)
  ds2 <- sig2 * (1 - sig2)
  ds3 <- sig3 * (1 - sig3)

  grad_a1 <- -2 * mean(err * beta1 * ds1)
  grad_a2 <- -2 * mean(err * beta2 * ds2)
  grad_a3 <- -2 * mean(err * beta3 * ds3)

  grad_b1 <- -2 * mean(err * beta1 * x * ds1)
  grad_b2 <- -2 * mean(err * beta2 * x * ds2)
  grad_b3 <- -2 * mean(err * beta3 * x * ds3)

  # parameter updates
  alpha <- alpha - lr * grad_alpha
  beta1 <- beta1 - lr * grad_beta1
  beta2 <- beta2 - lr * grad_beta2
  beta3 <- beta3 - lr * grad_beta3
  a1    <- a1    - lr * grad_a1
  a2    <- a2    - lr * grad_a2
  a3    <- a3    - lr * grad_a3
  b1    <- b1    - lr * grad_b1
  ...
  # check convergence
  MSE <- mean((Y - (alpha + beta1*sig1 + beta2*sig2 + beta3*sig3))^2)
...
# final fit
plot(x, Y, col="grey", main="Simulated data with fit")
lines(x, alpha + beta1*sig1 + beta2*sig2 + beta3*sig3, col="red", lwd=2)
```
...
```{r nn9, echo=FALSE, out.width = '65%', out.height='65%'}
...
<!--
```{r nn12,eval=FALSE, message=FALSE, warning=FALSE}
...
```{r deeplr1, cache=TRUE, message=FALSE, warning=FALSE}
library(keras)
library(tensorflow)
library(AER)
...
```{r Yigit, cache=TRUE, message=FALSE, warning=FALSE, eval=FALSE, include=FALSE, fig.cap="Training MSE and MAE"}
# REMOVED PLOT
# ```{r Yigit, cache=TRUE, message=FALSE, warning=FALSE}
...
```{python deepkeras2,warning=FALSE, message=FALSE}
...
```{python deepkeras3, cache=TRUE,warning=FALSE, message=FALSE}
...
```{python deepkeras4,warning=FALSE, message=FALSE}
score = model.evaluate(test_x, test_y, verbose=0)
...
```{python deepkeras5, results='hide',warning=FALSE, message=FALSE}
import pydot
...
```{r model-plot-display, echo=FALSE, out.width='100%'}
knitr::include_graphics("png/model_plot.png")
```
...
```{python deepkeras6, echo=FALSE, results='hide', fig.show='hide'}
import matplotlib
matplotlib.use("Agg")  # Use non-GUI backend safe for headless PDF rendering
...
plt.ylabel('Loss')
plt.legend()
plt.savefig("png/keras_loss_plot.png", bbox_inches='tight')
plt.close() 
```

```{r myplot, echo=FALSE, out.width='100%'}
knitr::include_graphics("png/keras_loss_plot.png")
```
...
```{python deepkeras7, cache=TRUE,warning=FALSE, message=FALSE}
...
```{r deepkeras8, warning=FALSE, message=FALSE, cache=TRUE}
library(keras)
library(dplyr)
...
```{r deepkeras9, cache=TRUE, message=FALSE, warning=FALSE}
library(keras)
library(tensorflow)
...
```{r deepkeras10, warning=FALSE, message=FALSE}
library(pROC)
